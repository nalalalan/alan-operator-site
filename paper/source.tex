...
The buyer-facing surface is intentionally plain. The current \texttt{relaybrief.com} page presents RelayBrief as a standalone manual follow-up cleanup service rather than as an AO Labs project route. The first screen now removes the payment decision from the first contact: the buyer emails one rough call note, receives a short plain-text preview, and pays USD~1 only if they want the finished packet. The page names the operator, identifies Stripe as the payment method after preview, and states that no card details, download, login, or password are entered on the site. These interface elements are not only conversion elements; they are also sensors. Opening the example output, emailing first, clicking the after-preview Stripe link, and entering paid intake all create evidence used by the proof contract.

Figure~\ref{fig:product_surface} shows the current production product surface used by the experiment. The desktop page presents only the domain, service, safety boundary, email-first preview path, after-preview Stripe route, and example output. The mobile view preserves the same order without adding a separate sales layout, file-download path, or card-entry form. The product surface is included here because future claims about buyer behaviour depend on what buyers actually saw.
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
\caption{Buyer-facing RelayBrief surface used by the evidence-locked deployment. The surface is both a product page and an experimental instrument: example opens, direct email, preview requests, paid intake, and checkout intent are recorded as buyer-intent signals.}
\label{fig:product_surface}
\end{figure}

The returned artifact is a follow-up packet. Figure~\ref{fig:sample_packet} shows the example output retained as the buyer-visible artifact. This matters because the experimental offer is not a vague artificial-intelligence claim; it is a concrete promise that rough sales-call notes can be converted into a recap, follow-up draft, next-step table, open questions, and CRM-ready update.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/relay-sample-preview.png}
\caption{RelayBrief example output. The revised sample makes the value proposition visible as a before--after transformation: rough call note received, finished handoff returned.}
...
Intent & 62 page views, 7 sample clicks, 2 note-intake clicks, 0 checkout clicks, 0 lead captures in the latest seven-day read & There was measurable site interaction. & Product-market fit or statistically reliable conversion. \\
...
First-money gates & The USD~1 price, product route, preview-first page, after-preview Stripe path, telemetry, sample inspection, paid-fulfillment check, and reply-window protection are present; the real paid-buyer gate remains missing. \\
Public offer path & Landing page, sample PDF, email-first preview path, after-preview Stripe path, lead API, payment-webhook preflight, reply-autoclose preflight, and outbound preflight were reported as configured or OK. \\
\bottomrule
\end{tabularx}
\end{table}
