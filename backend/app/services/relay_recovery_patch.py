...
RECOVERY_STEP_TEMPLATES = [
    ...
    StepTemplate(
        ...
        body=(
            ...
            "{sample_url}\n\n"
            "The test is one call by email: send one rough draft, get a short follow-up email preview first. No payment before preview. No software download, account, password, or card form.\n\n"
            "- Alan"
        ),
    ...
    StepTemplate(
        ...
        body=(
            "Last note from me.\n\n"
            "If one call is worth testing, reply with one rough draft. I will send a follow-up email preview before any payment.\n\n"
...
def _money_loop_success_sleep(result: dict[str, Any] | None, default_interval: int) -> tuple[int, str] | None:
    ...
    if bottleneck == "offer_market_rebuild_required":
        return min(default_interval, 300), "offer_rebuild_watch"
    if bottleneck in {"checkout_to_payment", "reply_to_payment", "active_signal_to_payment"}:
    ...
    for success_result in (success_after, success_before):
        ...
        if health_state in {
            "paid_fulfillment_open",
            "execution_proof_missed",
            "recovery_required",
            "buyer_signal_open",
            "active_signal_open",
        }:
        ...
        if health_state == "offer_rebuild_required":
            return min(default_interval, 300), "offer_rebuild_watch"
