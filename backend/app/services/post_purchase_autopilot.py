    ...
    return {
        "data": {
            ...
            "fields": [
                ...
                field("Paste your stuck client email, last reply, rough draft, or bullets", notes),
            ],
        }
    }
...
def run_messy_notes_checkout_followup_sweep(hours: int = 2) -> dict[str, Any]:
    ...
    for lead in send_candidates:
        try:
            external_id = f"relay-lead:{lead.id}"
            blocks = [
                _p("I have your stuck client email, last reply, or rough draft."),
                _p(
...
def run_sample_request_notes_followup_sweep(hours: int = 24) -> dict[str, Any]:
    ...
    for lead in send_candidates:
        try:
            ...
            _send_conversion_email(
                to_email=lead.email,
                subject="Have one stuck client email to test?",
                blocks=blocks,
    ...
    for event, lead in candidates:
        try:
            ...
            blocks = [
                ...
                _p(f"If you still want one stuck client email, last message, or rough draft turned into a finished follow-up email, the {entry_price_label()} link is here."),
                _a(_entry_packet_label(), _entry_packet_link()),
                _p("If you want to add or resend stuck client email reply context first, use the notes form."),
                _a("Send stuck client email reply", _notes_url()),
                _p("- Alan"),
            ]
            _send_conversion_email(
