...
def build_hot_reply_decision(text: str) -> HotReplyDecision:
    ...
    if any(x in t for x in ["call", "talk", "zoom", "phone", "meeting"]):
        return HotReplyDecision(
            ...
            reply_text=(
                ...
                f"{_ladder_block()}\n\n"
                "send one rough draft first and i will reply with the preview before any payment"
            ),
    ...
    if any(x in t for x in ["how much", "price", "pricing", "cost", "$"]):
        return HotReplyDecision(
            ...
            reply_text=(
                ...
                f"{_ladder_block()}\n\n"
                "send one rough draft first; i will include the payment link only with a useful preview"
            ),
    ...
    if any(x in t for x in ["sample", "proof", "scam", "what exactly", "what do i get", "how does it work"]):
        return HotReplyDecision(
            ...
            reply_text=(
                ...
                "send one rough draft first. i will reply with the preview before any payment."
            ),
    ...
    if any(x in t for x in ["later", "not now", "busy", "circle back", "next week"]):
        return HotReplyDecision(
            ...
            reply_text=(
                "makes sense\n\n"
                "lowest-friction option is one real call. send one rough draft first; i will reply with the preview before any payment."
            ),
