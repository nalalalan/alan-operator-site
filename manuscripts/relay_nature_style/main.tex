...
\vspace{0.35em}
\noindent\textbf{Autonomous agents are increasingly able to use tools, write messages, and execute workflows, but deployed claims of autonomy are difficult to evaluate when actions, constraints, market signals, and commercial outcomes are scattered across product analytics and human memory. We report Relay, an evidence-locked deployment of a notes-first revenue agent governed by an explicit proof contract. The system converts a broad commercial goal into a state machine with recorded bottlenecks, permitted actions, forbidden actions, proof deadlines, buyer-intent signals, payment outcomes, and compact research-journal entries. At the evidence lock, Relay exposed nine production routes, had all eleven checked environment requirements present, retained 30-day telemetry including 56 page views, 8 link clicks, 3 checkout clicks, and 3 lead-capture events, and recorded four compact research-journal entries. Crucially, it had not yet demonstrated autonomous profitability: gross revenue was USD~0.00, non-test payments were zero, and the active hard-paid-test experiment was 0 of 10 sends before its next business-hour window. The contribution is therefore not a claim that the system already makes money. It is a falsifiable protocol for making autonomous commercial experimentation auditable enough that future success or failure can be interpreted.}
...
Relay was built around a deliberately narrow offer: a buyer sends messy sales-call notes and receives a client-ready follow-up packet. The user-facing product is simple, but the research object is broader. Relay connects a product surface, buyer-intent telemetry, payment links, intake capture, outbound acquisition records, production-event routes, a success governor, an autonomous money loop, and a research-journal endpoint. The system is designed to keep the operator out of routine replanning during a proof window while preserving enough evidence to reconstruct what happened.

Here we frame Relay as a proof-governed autonomy system. The term denotes a closed loop in which the agent can act only under a recorded proof state: a measurable objective, a current bottleneck, a permitted next action, a forbidden-action set, a deadline, and outcome fields that define whether the action succeeded. This paper is evidence-locked to a saved production snapshot rather than to a live dashboard that can change after drafting. The resulting claim is intentionally bounded. Relay is deployed and auditable; it is not yet proven profitable.
...
Audit point & 2026-05-04T17:00:00-04:00 & Defines when the next evidence should be evaluated. \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\rowcolor{ClayLine}
\textbf{Observed outcome} & \textbf{Interpretation} & \textbf{Permitted next action} \\
\midrule
...
Before the journal layer, Relay could be monitored, but later reconstruction required multiple routes and human memory. The research-journal endpoint changes the status of the deployment because the system now records compact control entries. These entries are not motivational notes; they are evidence objects. They preserve the money state, active-sample progress, proof state, health state, and next action.
...
The autonomy policy was read from the success-controller and money-system summaries. The policy defines the current bottleneck, allowed next action, forbidden actions, proof deadline, owner-interruption state, and recovery conditions. These values are treated as experimental controls. A future protocol deviation should be recorded in the journal before the corresponding outcome is interpreted.

\subsection{Statistics and sample size}

No hypothesis test was performed. The available observations are descriptive: 56 page views, 18 filtered real page views, 8 link clicks, 3 checkout clicks, 3 lead-capture events, 1 filtered real lead, 60 rolling seven-day outbound sends, 0 replies, 0 payments, and 0 send failures. The active experiment target was 10 active sends, but the observed active sample at the evidence lock was 0. Future statistical claims require completed samples and a predeclared analysis plan.
