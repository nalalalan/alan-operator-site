...
The buyer-facing surface is intentionally plain. The current \texttt{relaybrief.com} page presents RelayBrief as a standalone manual follow-up cleanup service rather than as an AO Labs project route. The first screen now answers the trust problem before asking for payment: the operator is named, the price is USD~1, Stripe is identified as the payment method, and the page states that no card details, download, login, or password are entered on the site. The public page now exposes one primary action, \emph{Email one note}. Payment occurs only after Alan accepts the job and sends a Stripe link by email. These interface elements are not only conversion elements; they are also sensors. Opening the example output, emailing first, clicking the after-acceptance Stripe link, and entering paid intake all create evidence used by the proof contract.

Figure~\ref{fig:product_surface} shows the current production product surface used by the experiment. The desktop page presents only the domain, service, safety boundary, email-first contact path, after-acceptance Stripe route, and example output. The mobile view preserves the same order without adding a separate sales layout, file-download path, or card-entry form. The product surface is included here because future claims about buyer behaviour depend on what buyers actually saw.
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
\caption{Buyer-facing RelayBrief surface used by the evidence-locked deployment. The surface is both a product page and an experimental instrument: example opens, direct email, paid intake, and checkout intent are recorded as buyer-intent signals.}
...
Intent & 63 page views, 7 sample clicks, 2 note-intake clicks, 0 checkout clicks, 0 lead captures in the latest seven-day read & There was measurable site interaction. & Product-market fit or statistically reliable conversion. \\
...
The latest live check was read from the production Relay API at approximately 2026-05-18T06:58Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The previous \texttt{hard\_paid\_test\_direct} reply-observation window matured with zero replies and zero payments, and the controller rotated without operator choice into \texttt{stalled\_opportunity\_direct}. The active sample remains at 9/10. One additional active send is due in the next send window, and the next audit is 2026-05-18 at 5:00 PM Eastern. The current source revision also changes the product surface and preflight rule from pay-first to email-first/payment-after-acceptance because the observed no-payment state and repeated trust complaints made the payment-first page a poor first-money path.
...
Seven-day intent & 63 page views, 164 page-engagement events, 7 sample clicks, 2 note-intake clicks, 0 leads. \\
...
First-money gates & The USD~1 price, product route, email-first page, after-acceptance Stripe path, telemetry, sample inspection, paid-fulfillment check, and reply-window protection are present; the real paid-buyer gate remains missing. \\
Public offer path & Landing page, sample PDF, email-first path, after-acceptance Stripe path, lead API, payment-webhook preflight, reply-autoclose preflight, and outbound preflight were reported as configured or OK. \\
\bottomrule
\end{tabularx}
\end{table}
