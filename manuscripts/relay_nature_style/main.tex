...
The buyer-facing surface is intentionally plain. The current \texttt{relaybrief.com} page presents RelayBrief as a standalone manual follow-up email service rather than as an AO Labs project route. The first screen now removes checkout from the first contact: the buyer emails rough notes from one call, receives one follow-up-email preview, and pays USD~1 only if the email is useful. The page names the operator, states that the Stripe link is sent with the preview, and states that no card details, download, login, or password are entered on the site. These interface elements are not only conversion elements; they are also sensors. Opening the example output and emailing first create evidence used by the proof contract; payment is measured only after a preview exists.

Figure~\ref{fig:product_surface} shows the current production product surface used by the experiment. The desktop page presents only the domain, service, safety boundary, email-first preview path, no-checkout-before-preview payment rule, and example output. The mobile view preserves the same order without adding a separate sales layout, file-download path, or card-entry form. The product surface is included here because future claims about buyer behaviour depend on what buyers actually saw.
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
\caption{Buyer-facing RelayBrief surface used by the evidence-locked deployment. The surface is both a product page and an experimental instrument: example opens, direct email, preview requests, paid intake, and payment-after-preview behaviour are recorded as buyer-intent signals.}
...
Intent & 61 page views, 6 sample clicks, 2 note-intake clicks, 0 checkout clicks, 0 lead captures in the latest seven-day read & There was measurable site interaction. & Product-market fit or statistically reliable conversion. \\
...
The latest live check was read from the production Relay API at approximately 2026-05-18T09:15Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The previous \texttt{hard\_paid\_test\_direct} reply-observation window matured with zero replies and zero payments, and the controller rotated without operator choice into \texttt{stalled\_opportunity\_direct}. The active sample remains at 9/10. One additional active send is due in the next send window, and the next audit is 2026-05-18 at 5:00 PM Eastern. The current source revision changes the product surface and preflight rule from pay-first and acceptance-first payment to preview-first/payment-after-useful because the observed no-payment state and repeated trust complaints made asking for money before value a poor first-money path.
...
\midrule
One-day intent & 18 page views, 63 page-engagement events, 2 sample clicks, 0 leads. \\
Seven-day intent & 61 page views, 164 page-engagement events, 7 sample clicks, 2 note-intake clicks, 0 leads. \\
...
First-money gates & The USD~1 price, product route, one-email preview-first page, no-checkout-before-preview rule, telemetry, sample inspection, paid-fulfillment check, and reply-window protection are present; the real paid-buyer gate remains missing. \\
Public offer path & Landing page, sample PDF, email-first preview path, payment-after-preview rule, lead API, payment-webhook preflight, reply-autoclose preflight, and outbound preflight were reported as configured or OK. \\
\bottomrule
\end{tabularx}
\end{table}
...
No hypothesis test was performed. The initial evidence-lock observations were descriptive: 56 page views, 18 filtered real page views, 8 link clicks, 3 checkout clicks, 3 lead-capture events, 1 filtered real lead, 60 rolling seven-day outbound sends, 0 replies, 0 payments, and 0 send failures. The latest live operating observations are also descriptive: 61 seven-day page views, 6 sample clicks, 2 note-intake clicks, 0 leads, 0 replies, 0 payments, and USD~0.00 gross revenue. The active outbound state is \texttt{stalled\_opportunity\_direct} at 9/10 progress with one active send expected in the next window, so the current observation is still an execution and trust-friction finding rather than evidence of demand failure. The dashboard readiness state is an implementation-readiness measure, not a revenue result; the only revenue endpoint remains a non-owner Stripe payment. Future statistical claims require completed samples, matured observation windows, and a predeclared analysis plan.
