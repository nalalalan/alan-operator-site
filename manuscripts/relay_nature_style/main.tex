...
The buyer-facing surface is intentionally plain. The current \texttt{relaybrief.com} page presents RelayBrief as a standalone paid service rather than as an AO Labs project route. The first screen now answers the trust problem before asking for payment: the domain is named, the price is USD~1, Stripe is identified as the checkout path, and the page states that no download, install, account, or password is required. A visitor can continue to Stripe, email first, expand an inline sample description, or open a lower-priority notes form if notes are already available. These interface elements are not only conversion elements; they are also sensors. Opening the sample disclosure, focusing the note field, submitting notes, clicking checkout, and entering an email all create evidence used by the proof contract.

Figure~\ref{fig:product_surface} shows the current production product surface used by the experiment. The desktop page presents only the domain, service, safety boundary, payment route, contact, Stripe action, inline sample disclosure, and collapsed notes form. The mobile view preserves the same order without adding a separate sales layout or file-download path. The product surface is included here because future claims about buyer behaviour depend on what buyers actually saw.
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
\caption{Buyer-facing RelayBrief surface used by the evidence-locked deployment. The surface is both a product page and an experimental instrument: note-intake focus, sample disclosure opens, lead capture, and checkout intent are recorded as buyer-intent signals.}
\label{fig:product_surface}
\end{figure}

The returned artifact is a follow-up packet. Figure~\ref{fig:sample_packet} shows the sample packet retained as the buyer-proof artifact. This matters because the experimental offer is not a vague artificial-intelligence claim; it is a concrete promise that rough sales-call notes can be converted into a recap, follow-up draft, next-step table, open questions, and CRM-ready update.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/relay-sample-preview.png}
\caption{Relay sample-packet proof artifact. The revised sample makes the value proposition visible as a before--after transformation: rough call note received, finished handoff returned.}
